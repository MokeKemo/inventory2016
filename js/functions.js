...
//*********************************************************************
//********** Odavde pocinje ono sto smo mi pisali *********************
//*********************************************************************
...
function deleteRow(table,obj,rowNum) {
    ...
    if (r == true) {
        ...
        var podaci = {action:"deleteInv",id:idNum,table:table};
...
function addToInventory(){
    ...
    podaciForme['action'] = 'addInv';

    podaciForme['user'] = $("#user").attr('data-id');
    ...
    var categoryID = podaciForme["category"];
    var categoryName = $('#category > option[value=' + categoryID + ']').html();
    var supplierID = podaciForme["supps"];
    var supplierName = $('#suppliers > option[value=' + supplierID + ']').html();
    var statusID = podaciForme["status"];
    var statusName = $('#status > option[value=' + statusID + ']').html();

    var userName= $('#user').val();
//console.log(counter);

    /*if (podaciForme["title"] == "" || podaciForme["description"] == "" || podaciForme["quantity"] == "" || podaciForme["category"] == "" || podaciForme["serialNumber"] == "" || podaciForme["price"] == "" || podaciForme["supps"] == "" || podaciForme["date"] == "" || podaciForme["user"] == "" || podaciForme["status"] == ""){
...
//console.log(podaciForme);
...

            {
                ...
                $('#tabela').append('<tr id="r'+incrementedLastRowInTable+'">' +
                    ...
                    '<td >'+ podaciForme["description"] +'</td>'+

                    '<td >'+ podaciForme["quantity"] +'</td>'+

                    '<td >'+ categoryName +'</td>'+

                    '<td >'+ podaciForme["serialNumber"] +'</td>'+

                    '<td >'+ podaciForme["invoicelNumber"] +'</td>'+

                    '<td >'+ podaciForme["price"] +'</td>'+

                    '<td >'+ supplierName +'</td>'+

                    '<td >'+ podaciForme["date"] +'</td>'+

                    '<td >'+ userName +'</td>'+

                    '<td >'+ statusName +'</td>'+

                    '<td ><button type="button" data-id="'+data+'" class="delButton" style="width:100px;font-size: 12px;" onclick="deleteRow(\'inventorytbl\',this,\'r'+ incrementedLastRowInTable +'\');";">Delete</button></td>'+
                    '<td ><button type="button"  class="openPopup" data-id="'+data+'" style="width:100px;height:30px;font-size: 12px;" onclick="showPopupFunction('+data+')"><strong>Preview</strong></button></td>'+
...
var table = "inventorytbl"; // OBAVEZNO PRILAGODJAVANJE TABELI
...
//odavde krece validacija forme


$(function(){
    $("#addToInv").click(function(){
        var validateForm = $("#forma").parsley().validate();
        if(validateForm){
...
function deleteFromSearch(idField, counter){

    var r = confirm("Are you shure you want to delete the record?");

    if(r == true){

        var podaci = {action:"deleteInv",id:idField,table:'inventorytbl'};
